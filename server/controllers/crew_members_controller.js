...
const createCrewMember = async (req, res) => {
  ...
  try {
    ...
    if  (err.name === 'SequelizeUniqueConstraintError') {
      return res.status(402).json({ message: 'The crew member already exists'});
    }
...
const updateCrewMember = async (req, res) => {
  ...
  try {
    ...
    if(!crewMemberToUpdate) {
      return res.status(402).json({message: 'The crew member you are trying to update does not exists'})
    }
    return res.status(402).json({error: err})
  }
...
const deleteCrewMember = async(req, res) => {
  ...
  try{
    ...
    if(!crewMemberToDelete) {
      return res.status(402).json({message: 'The crew member you are trying to delete does not exists'})
    }
    return res.status(402).json({error: err})
  }
